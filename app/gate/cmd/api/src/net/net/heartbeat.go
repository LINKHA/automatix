...
func (r *HeatBeatDefaultRouter) Handle(req iface.IRequest) {
	log.Ins().InfoF("Recv Heartbeat from %s, MsgID = %+v, Data = %s",
...
func notAliveDefaultFunc(conn iface.IConnection) {
	log.Ins().InfoF("Remote connection %s is not alive, stop it", conn.RemoteAddr())
	conn.Stop()
}
...
func (h *HeartbeatChecker) Stop() {
	log.Ins().InfoF("heartbeat checker stop, connID=%+v", h.conn.GetConnID())
	h.quitChan <- true
}
...
func (h *HeartbeatChecker) SendHeartBeatMsg() error {
	...
	if err != nil {
		log.Ins().ErrorF("send heartbeat msg error: %v, msgId=%+v msg=%+v", err, h.msgID, msg)
		return err
	}
