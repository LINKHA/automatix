...
// The package is named "conf", and the file is named "globalobj.go".
...
func (g *Config) InitLogConfig() {
	if g.LogFile != "" {
		...
		log.SetCons(g.LogCons)
	}
	if g.LogSaveDays > 0 {
		log.SetMaxAge(g.LogSaveDays)
	}
	if g.LogFileSize > 0 {
		log.SetMaxSize(g.LogFileSize)
	}
	if g.LogIsolationLevel > log.LogDebug {
		log.SetLogLevel(g.LogIsolationLevel)
	}
}
