...
import "github.com/LINKHA/automatix/common/log"
...
func UserConfToGlobal(config *Config) {
	...
	if GlobalObject.LogIsolationLevel > log.LogDebug {
		log.SetLogLevel(GlobalObject.LogIsolationLevel)
	}
	...
	if config.LogFile != "" {
		...
		log.SetLogFile(GlobalObject.LogDir, GlobalObject.LogFile)
	}
