...
func useWorker(conn ziface.IConnection) uint32 {
	...
	if mh == nil {
		log.Ins().ErrorF("useWorker failed, mh is nil")
		return 0
	}
...
func (mh *MsgHandle) SendMsgToTaskQueue(request ziface.IRequest) {
	...
	// log.Ins().DebugF("Add ConnID=%d request msgID=%d to workerID=%d", request.GetConnection().GetConnID(), request.GetMsgID(), workerID)
	// Send the request message to the task queue
	mh.TaskQueue[workerID] <- request
	log.Ins().DebugF("SendMsgToTaskQueue-->%s", hex.EncodeToString(request.GetData()))
}
...
func (mh *MsgHandle) doFuncHandler(request ziface.IFuncRequest, workerID int) {
	defer func() {
		if err := recover(); err != nil {
			log.Ins().ErrorF("workerID: %d doFuncRequest panic: %v", workerID, err)
		}
	}()
...
func (mh *MsgHandle) doMsgHandler(request ziface.IRequest, workerID int) {
	...
	if !ok {
		log.Ins().ErrorF("api msgID = %d is not FOUND!", request.GetMsgID())
		return
	}
...
func (mh *MsgHandle) AddRouter(msgID uint32, router ziface.IRouter) {
	...
	log.Ins().InfoF("Add Router msgID = %d", msgID)
}
...
func (mh *MsgHandle) StartOneWorker(workerID int, taskQueue chan ziface.IRequest) {
	log.Ins().InfoF("Worker ID = %d is started.", workerID)
