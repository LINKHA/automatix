...
func (s *Server) ListenTcpConn() {
	...
	if err != nil {
		log.Ins().ErrorF("[START] resolve tcp addr err: %v\n", err)
		return
	}
	...
	go func() {
		for {
			...
			if s.ConnMgr.Len() >= zconf.GlobalObject.MaxConn {
				log.Ins().InfoF("Exceeded the maxConnNum:%d, Wait:%d", zconf.GlobalObject.MaxConn, AcceptDelay.duration)
				AcceptDelay.Delay()
				continue
			}
			...
			if err != nil {
				...
				if errors.Is(err, net.ErrClosed) {
					log.Ins().ErrorF("Listener closed")
					return
				}
				log.Ins().ErrorF("Accept err: %v", err)
				AcceptDelay.Delay()
				continue
			}
	...
	case <-s.exitChan:
		err := listener.Close()
		if err != nil {
			log.Ins().ErrorF("listener close err: %v", err)
		}
	}
}
...
func (s *Server) ListenWebsocketConn() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if s.websocketAuth != nil {
			...
			if err != nil {
				log.Ins().ErrorF(" websocket auth err:%v", err)
				w.WriteHeader(401)
				AcceptDelay.Delay()
				return
			}
		...
		if err != nil {
			log.Ins().ErrorF("new websocket err:%v", err)
			w.WriteHeader(500)
			AcceptDelay.Delay()
			return
		}
...
func (s *Server) ListenKcpConn() {
	...
	if err != nil {
		log.Ins().ErrorF("[START] resolve KCP addr err: %v\n", err)
		return
	}

	log.Ins().InfoF("[START] KCP server listening at IP: %s, Port %d, Addr %s", s.IP, s.KcpPort, listener.Addr().String())
	...
	go func() {
		for {
			...
			conn, err := listener.Accept()
			if err != nil {
				log.Ins().ErrorF("Accept KCP err: %v", err)
				AcceptDelay.Delay()
				continue
			}
	...
	case <-s.exitChan:
		err := listener.Close()
		if err != nil {
			log.Ins().ErrorF("KCP listener close err: %v", err)
		}
	}
}
...
// (开启网络服务)
func (s *Server) Start() {
	log.Ins().InfoF("[START] Server name: %s,listener at IP: %s, Port %d is starting", s.Name, s.IP, s.Port)
...
func (s *Server) Stop() {
	log.Ins().InfoF("[STOP] Zinx server , name %s", s.Name)
...
func (s *Server) Serve() {
	...
	sig := <-c
	log.Ins().InfoF("[SERVE] Zinx server , name %s, Serve Interrupt, signal = %v", s.Name, sig)
}
