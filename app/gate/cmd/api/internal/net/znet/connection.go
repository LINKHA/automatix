...
import (
	...
	"automatix/app/gate/cmd/api/internal/net/zconf"
	"automatix/app/gate/cmd/api/internal/net/zinterceptor"
...
func (c *Connection) StartWriter() {
	log.Ins().InfoF("Writer Goroutine is running")
	defer log.Ins().InfoF("%s [conn Writer exit!]", c.RemoteAddr().String())
		...
		case data, ok := <-c.msgBuffChan:
			if ok {
				if err := c.Send(data); err != nil {
					log.Ins().ErrorF("Send Buff Data error:, %s Conn Writer exit", err)
					break
				}

			} else {
				log.Ins().ErrorF("msgBuffChan is Closed")
				break
			}
		case <-c.ctx.Done():
			return
...
func (c *Connection) StartReader() {
	log.Ins().InfoF("[Reader Goroutine is running]")
	defer log.Ins().InfoF("%s [conn Reader exit!]", c.RemoteAddr().String())
	defer c.Stop()
	defer func() {
		if err := recover(); err != nil {
			log.Ins().ErrorF("connID=%d, panic err=%v", c.GetConnID(), err)
		}
	}()
	...
	for {
		...
		default:
			...
			if err != nil {
				log.Ins().ErrorF("read msg head [read datalen=%d], error = %s", n, err)
				return
			}
			log.Ins().DebugF("read buffer %s \n", hex.EncodeToString(buffer[0:n]))
...
func (c *Connection) Start() {
	defer func() {
		if err := recover(); err != nil {
			log.Ins().ErrorF("Connection Start() error: %v", err)
		}
	}()
...
func (c *Connection) Send(data []byte) error {
	...
	if err != nil {
		log.Ins().ErrorF("SendMsg err data = %+v, err = %+v", data, err)
		return err
	}
...
func (c *Connection) SendToQueue(data []byte) error {
	...
	if data == nil {
		log.Ins().ErrorF("Pack data is nil")
...
func (c *Connection) SendMsg(msgID uint32, data []byte) error {
	...
	if err != nil {
		log.Ins().ErrorF("Pack error msg ID = %d", msgID)
		return errors.New("Pack error msg ")
	}

	err = c.Send(msg)
	if err != nil {
		log.Ins().ErrorF("SendMsg err msg ID = %d, data = %+v, err = %+v", msgID, string(msg), err)
		return err
	}
...
func (c *Connection) finalizer() {
	...
	log.Ins().InfoF("Conn Stop()...ConnID = %d", c.connID)
}

func (c *Connection) callOnConnStart() {
	if c.onConnStart != nil {
		log.Ins().InfoF("ZINX CallOnConnStart....")
		c.onConnStart(c)
	}
}
...
func (c *Connection) callOnConnStop() {
	if c.onConnStop != nil {
		log.Ins().InfoF("ZINX CallOnConnStop....")
		c.onConnStop(c)
	}
}
